...
describe("[Exercise 6] Car", () => {
  ...
  test("[15] driving the car returns the updated odometer", () => {
    ...
    expect(focus.drive(200)).toBe(`${600} (ran out of gas)`);
    expect(focus.drive(200)).toBe(`${600} (gas tank is empty, must refuel)`);
  });
  test("[16] driving the car uses gas", () => {
    focus.drive(600);
    expect(focus.drive(300)).toBe(`${600} (gas tank is empty, must refuel)`);
    expect(focus.drive(1000)).toBe(`${600} (gas tank is empty, must refuel)`);
    expect(focus.tank).toBe(0);
  });
  test("[17] refueling allows to keep driving", () => {
    expect(focus.drive(600)).toBe(`${600} (ran out of gas)`);
...
describe("[Exercise 7] isEvenNumberAsync", () => {
  ...
  test("[21] rejects an error with the message 'number must be a number type' ", async () => {
    try {
      await utils.isEvenNumberAsync("foobar");
    } catch (error) {
      expect(error.message).toMatch(/number must be a type: number/i);
    }
  });
});
